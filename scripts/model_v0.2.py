...
def main(mesh: bool = True, model: bool = True, results=True):
    ...
    seaset_path = "v0.2/seaset_full.csv"
    ...
    if model:
        for solver in ["schism", "telemac"]:
            ...
            b.create()
            # clip depths to -20m
            z = b.mesh.Dataset.depth
            z[z < 20] = 20
            b.mesh.Dataset["depth"] = xr.Variable(("nSCHISM_hgrid_node"), z)
            #
            b.output()
            fix_mesh(b)
            b.mesh.to_file(f"v0.2/{solver}/hgrid.gr3")
            b.save()
            b.set_obs()
            b.run()
